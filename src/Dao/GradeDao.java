	...
	public ArrayList<Grade> find(String stuId){
		...
		try{
			...
			while (rs.next()) {
				...
				grade.setTcId(rs.getString("tcId"));
				grade.setClassId(rs.getString("classId"));
				System.out.println(rs.getString("courseGrade"));
				list.add(grade);
			}
			return list;
		}catch(Exception ee){
...
//	public ArrayList<Grade> find(String stuId, String tcId){
//		ArrayList<Grade> list = new ArrayList<Grade>();
//		try{
//			conn = JDBCUtils.getConnection();
//			stmt = conn.createStatement();
//			String sql = "select * from grade where stuId = '" + stuId + "'+ and tcId = '"+ tcId +"'";
//			rs = stmt.executeQuery(sql);
//			System.out.println("单个查询SQL语句：" + sql);
//			while (rs.next()) {
//				Grade grade = new Grade();
