		...
		<c:when test="${sessionScope.user.identity == 'admin'}">
			<div class="tit">
				...
				<form action="${pageContext.request.contextPath }/FindScoreServlet" method="post">
					...
					<input name="tcId" type="hidden" value="<%=request.getParameter("tcId")%>">
					<h2>成绩查询方式：学号查询</h2><br/>
					<input name="id" type="text"><span>${errerMsg}</span>
			...
			<table align="center" cellpadding="5" cellspacing="0" border="1">
				...
				<tr>
					<td colspan="8" align="center">学号为
						<%=list.get(0).getStuId() %>的学生成绩如下</td>
				</tr>
				<tr>
					<td>学生学号</td>
					<td>学生姓名</td>
				...
				<tr>
					...
					<form action="${pageContext.request.contextPath }/UpdScoreServlet" method="post">
						<td><input type="hidden" name="stuId" value="<%=list.get(j).getStuId() %>">
							...
							<input name="grade" type="text" value="<%=list.get(j).getCourseGrade()%>"></td>
						<td><input type="submit" value="修改" class="btn upd"></td>
					</form>
					<form action="${pageContext.request.contextPath }/DelScoreServlet" method="post">
						...
						<td><input type="submit" value="删除" class="btn del"></td>
					</form>
				</tr>
				<% }}%>
			</table>
